...
$(function(){
    ...
    $("#addForm").validate({
        ...
        submitHandler: function (form) {
            ...
            $.ajax({
                type: "POST",
                url: path+"/admin/systemSetting/helpCenterConf/createArticleInfo",
                data: {
                    'smArticleClManage.id': $('#articleClass2').val(),
                    'title': $('#title').val(),
                    'articleType': $('input[name="articleType"]:checked').val(),
                    'content': UE.getEditor('content').getContent()//$('#content').val()
                },
                dataType: 'json',
                success: function (json) {
                    $("#projectType").dialog("close");
