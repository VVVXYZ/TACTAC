...
$(function(){
    ...
    $("#editForm").validate({
        ...
        submitHandler: function (form) {
            ...
            $.ajax({
                type: "POST",
                url: path+"/admin/systemSetting/helpCenterConf/updateArticleInfo",
                data: {
                    'id': $('#id2').val(),
                    'smArticleClManage.id': $('#articleClass22').val(),
                    'title': $('#title2').val(),
                    'articleType': $('input[name="articleType2"]:checked').val(),
                    'content': UE.getEditor('content2').getContent()//$('#content2').val()
                },
                dataType: 'json',
                success: function (json) {
                    $("#projectType2").dialog("close");
