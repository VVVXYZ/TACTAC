...
public class SimpleSMTPSender {
    ...
    public void setProperties(String senderName, String receiverList) throws MessagingException {
        ...
        String nick = "";//设置发件人昵称
        try {
            nick = javax.mail.internet.MimeUtility.encodeText(senderName);
        } catch (UnsupportedEncodingException e) {
            e.printStackTrace();
        }
        //msg.setFrom(new InternetAddress(nick+" <"+from+">"));
        message.setFrom(new InternetAddress(nick + " <" + senderEmail + ">"));
        allReceiver = InternetAddress.parse(receiverList);
        message.setRecipients(Message.RecipientType.TO, allReceiver);
